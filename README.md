...
**The Claude desktop app.** Its **Code** tab needs nothing extra — it shares `~/.claude` with the CLI, so the plugin installed above already covers it (start a new session, not just a relaunch). Its **chat** surface, like claude.ai and mobile, takes account-level connectors rather than local config, and only you can add one — so when the app is present otx prints a prefilled `add-custom-connector` link and writes nothing. Earlier versions wrote an `mcp-remote` stdio bridge into `claude_desktop_config.json`; that put your key in an npx process's argv (readable via `ps`), needed Node on PATH, and made Code-tab sessions list OpenTrace twice. otx now removes such an entry when it finds one.
...
A key that expires or is revoked is otx's problem, not yours to watch for: the [notice banner](#notices) tells you the next time you run any command, and names the fix. You do not need to recognise a `401`.
